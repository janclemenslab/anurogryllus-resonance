...
def lifac_all(time, outs, params_file):
    ppf = []
    V = []
    A = []
    S = []
    for out in outs[0].T:
        adapt_spikes, v, a = lifac(
            time,
            out,
            taum=params_file[5],
            tref=params_file[6],
        ...
        ppf.append(len(adapt_spikes))
        V.append(v)
        A.append(a)
        S.append(adapt_spikes)
    return np.array(ppf), np.array(V), np.array(A), S
